...
#
# ============================================================================
# MODIFIED: language conditioning was previously read from the data
# (`item.get('language', 'Auto')`) but never actually injected into the
# training sequence -- meaning the model never learned which language it was
# synthesizing, and the codec_language_id embedding for any newly added
# language (e.g. Arabic, ID 2072) never received gradient updates.
#
# Changes marked with "# CHANGED" below:
#   1. __getitem__ now resolves the language string to its codec ID via
#      config.talker_config.codec_language_id (case-insensitive) and
#      returns it as part of the sample.
#   2. collate_fn now builds a 6-token codec preamble (was 5 tokens) that
#      includes the resolved lang_id, using codec_think_id/think_bos_id/
#      think_eos_id instead of codec_nothink_id. This shifts every
#      subsequent offset in the sequence by +1 (8 -> 9).
# ============================================================================
...
class TTSDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        speaker_name    = item.get('speaker')  # CHANGED: needed to snapshot a per-speaker embedding later
        if not speaker_name:
            raise ValueError(
                "Every training sample must have a 'speaker' field "
                "(e.g. 'mahmoud', 'sara') so multiple speakers can be "
                "trained together in one run and each gets its own "
                "named-voice embedding at checkpoint time."
            )
        ...
        return {
            ...
            "speaker_name": speaker_name,   # CHANGED: carried through to collate_fn
        }
    ...
    def collate_fn(self, batch):
        ...
        # CHANGED: plain python list, not a tensor -- one speaker name per
        # sample in the batch, same order as ref_mels / speaker_embedding.
        ...
        return {
            ...
            'speaker_names': speaker_names,   # CHANGED
        }